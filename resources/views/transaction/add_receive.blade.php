...
<div class="container">
    ...
    <div class="row justify-content-center">
        <div class="col-md-8">
            <div class="card">
                <div class="card-header">{{ __('Add New Receive Transaction') }}</div>

                <div class="card-body">
                    @if($wallets->isEmpty())
                    <p class="text-center">You don't have any wallet. <a href="{{ route('wallet.showAdd') }}" title="">Create one</a>.</p>
                    @elseif($categories->isEmpty())
                    <p class="text-center">You don't have any receive category. <a href="{{ route('category.showAddReceive') }}" title="">Create one</a>.</p>
                    @else
                    <form method="POST" action="{{ route('transaction.addReceive') }}" aria-label="{{ __('Add New Receive Transaction') }}">
                        @csrf
